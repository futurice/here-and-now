...
public class NewTopicActivity extends NewCardActivity {
    ...
    @Override
    protected void onCreate(@NonNull @nonnull final Bundle savedInstanceState) {
    ...
    private void postImageMessage() {
        if (mediaUri.getPath() == null || mediaUri.getPath().equals("")) {
            ...
            File pictureFile = FileUtils.getFile(this, mediaUri); // Returns null and causes a crash if the file is not local
            if (pictureFile == null) {
                Toast toast = Toast.makeText(this, R.string.error_non_local_file, Toast.LENGTH_LONG);
                toast.show();
                return;
            }
    ...
    private void sendCardToService(@NonNull @nonnull final File pictureFile) {
